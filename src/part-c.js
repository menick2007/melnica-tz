...
const PORES = [
  ...
  { v: "На усмотрение изготовителя", ico: "free" }
];
const CUTS = [
...
const SPEC = [
  ...
  { k:"p_bake", lb:"Режим выпечки после дефростации. Программа выпекания, если установлена", req:2, t:"area", span:2,
    ...
    chips:["На усмотрение изготовителя"],
    note:"Если режимы не заданы — нажмите «На усмотрение изготовителя».",
  ...
  { k:"p_h", lb:"Высота, см", req:2, t:"textchips", ph:"5–6", chips:["На усмотрение изготовителя"],
    hint:"Точно либо ориентировочно. Пример: 5–6 см.\nЕсли размер не принципиален — нажмите «На усмотрение изготовителя»." },
  { k:"p_l", lb:"Длина, см", req:2, t:"textchips", ph:"48–50", chips:["На усмотрение изготовителя"],
    hint:"Точно либо ориентировочно. Пример: 48–50 см.\nЕсли размер не принципиален — нажмите «На усмотрение изготовителя»." },
  { k:"p_w", lb:"Ширина, см", req:2, t:"textchips", ph:"10–12", chips:["На усмотрение изготовителя"],
    hint:"Точно либо ориентировочно. Пример: 10–12 см.\nЕсли размер не принципиален — нажмите «На усмотрение изготовителя»." },
...
const ICONS = {
  ...
  free:  '<svg viewBox="0 0 100 100" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"><path d="M34 59v15a3 3 0 0 0 3 3h26a3 3 0 0 0 3-3V59"/><path d="M34 59a12 12 0 1 1 3.6-23.4A13 13 0 0 1 50 26a13 13 0 0 1 12.4 9.6A12 12 0 1 1 66 59Z"/><path d="M34 67h32"/></svg>'
};
...
function swOpt(f, o, name) {
  const id = uid();
  const media = o.g
    ? '<span class="sw-c" style="background:' + o.g + '"></span>'
    : '<span class="sw-c free">' + (ICONS[o.ico] || "") + "</span>";
  return '<label class="opt" for="' + id + '">' + media +
    '<span class="opt-cap">' + esc(o.v) + "</span>" +
    '<span class="opt-ck"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.4" stroke-linecap="round" stroke-linejoin="round"><path d="m20 6-11 11-5-5"/></svg></span>' +
...
/* Дата заполнения проставляется при первом сохранении черновика и дальше едет
   вместе с данными — в файле заявки, в документе, в таблице. Поэтому в
   утверждённом ФС стоит день, когда заявку заполнил клиент, а не день,
   когда менеджер открыл файл. */
let filledAt = null;
function fillDate() {
  const d = filledAt ? new Date(filledAt) : new Date();
...
function apply(d) {
  if (!d) return;
  filledAt = d.filledAt || d.ts || filledAt;
...
function save() {
  if (!filledAt) filledAt = new Date().toISOString();
  const el = $("#saved");
...
/* Блок клиента во всех выгрузках начинается с даты заполнения. */
function clientRows() {
  return [{ lb: "Дата заполнения", text: fillDate() }].concat(humanList($("#clientBody"), CLIENT));
}
...
async function flatPayload() {
  ...
  out.push({ title: "Клиент", rows: await conv(clientRows()) });
...
function buildDoc() {
  ...
  const html = '<!doctype html><html lang="ru"><head><meta charset="utf-8">' +
    ...
    "<section><h2><i>1</i> Клиент</h2>" + rowsHTML(clientRows()) +
  ...
  return { html: html, name: fileBase() + ".html" };
}

/* Имя файла общее для всех выгрузок: html-заявка, таблица Excel, csv. */
function fileBase() {
  const cName = readField($('#clientBody .f[data-k="cl_name"]')).text || "Без названия";
  const mgStat = readField($('#mgrBody .f[data-k="mg_status"]')).text;
  const approved = MODE === "manager" && mgStat === "Согласовал";
  return "ТЗ_" + cName.replace(/[\\/:*?"<>|]+/g, " ").trim().slice(0, 60) +
    "_" + new Date().toISOString().slice(0, 10) + (approved ? "_утверждено" : "");
}
...
/* ============================================================
   ВЫГРУЗКА В EXCEL
   .xlsx собирается прямо в браузере: минимальный zip без сжатия
   плюс несколько xml-частей. Библиотек нет — страница остаётся
   одним самодостаточным файлом. Фотографии в таблицу не идут:
   нужна форма с графами, а снимки лежат в файле заявки и в PDF.
   ============================================================ */
...
/* zip методом «store»: содержимое кладётся как есть, без deflate —
   xml сжимать нечем, зато не нужна библиотека компрессии. */
function zipStore(files) {
  ...
  let p = 0;
  all.forEach(chunk => { out.set(chunk, p); p += chunk.length; });
  return out;
}
...
function xe(s) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (c === 13 || c === 10) {                       // перенос строки внутри ячейки
      if (c === 13 && str.charCodeAt(i + 1) === 10) i++;
      out += "&#10;";
      continue;
    }
    if (c < 32) continue;                             // прочие управляющие символы xml не принимает
    const ch = str.charAt(i);
    out += ch === "&" ? "&amp;" : ch === "<" ? "&lt;" : ch === ">" ? "&gt;" : ch === String.fromCharCode(34) ? "&quot;" : ch;
  }
  return out;
}
function xlCell(ref, style, text) {
  if (text == null || text === "") return '<c r="' + ref + '" s="' + style + '"/>';
  return '<c r="' + ref + '" s="' + style + '" t="inlineStr"><is><t xml:space="preserve">' + xe(text) + "</t></is></c>";
}

/* Все графы блока подряд — в отличие от документа, пустые тоже:
   в Excel уезжает форма целиком, чтобы её было видно как бланк. */
function excelRows(host, list) {
  const rows = [];
  list.forEach(f => {
    if (f.h) { rows.push({ k: "sub", a: f.h }); return; }
    const fEl = $('.f[data-k="' + f.k + '"]', host);
    if (!fEl) return;
    const r = readField(fEl);
    if (f.t === "files") {
      const n = (r.files || []).length;
      rows.push({ k: "pair", a: f.lb, b: n ? n + " шт. — вложены в файл заявки" : "" });
      return;
    }
    rows.push({ k: "pair", a: f.lb, b: r.text, danger: f.danger });
  });
  return rows;
}
...
function excelModel() {
  ...
  const R = [];
  R.push({ k: "title", a: "Техническое задание на разработку продукции" });
  R.push({ k: "meta", a: cName + " · сформировано " + date });
  if (MODE === "manager" && mgStat) R.push({ k: "meta", a: [mgStat, mgName, mgDate].filter(Boolean).join(" · ") });

  R.push({ k: "gap" });
  R.push({ k: "sec", a: "1. Клиент" });
  R.push({ k: "pair", a: "Дата заполнения", b: fillDate() });
  excelRows($("#clientBody"), CLIENT).forEach(r => R.push(r));

  const mg = excelRows($("#mgrBody"), MANAGER);
  if (MODE === "manager" || mg.some(r => r.b)) {
    R.push({ k: "sub", a: "Служебное — заполняет менеджер Мельницы" });
    mg.forEach(r => R.push(r));
  }

  $$("#items .item").forEach((b, i) => {
    const nm = readField($('.f[data-k="p_name"]', b)).text || "без наименования";
    R.push({ k: "gap" });
    R.push({ k: "sec", a: "Изделие " + (i + 1) + " — " + nm });
    R.push({ k: "sub", a: "2. Характеристика" });
    excelRows($(".body-spec", b), SPEC).forEach(r => R.push(r));
    R.push({ k: "sub", a: "3. Фото и референс" });
    excelRows($(".body-photo", b), PHOTOS).forEach(r => R.push(r));
    R.push({ k: "sub", a: "4. Хранение и упаковка" });
    excelRows($(".body-store", b), STORAGE).forEach(r => R.push(r));
  });
  return R;
}
...
const XL_STYLES =
  ...
  '<fonts count="6">' +
    '<font><sz val="11"/><color rgb="FF15211A"/><name val="Calibri"/></font>' +
    '<font><b/><sz val="16"/><color rgb="FF15211A"/><name val="Calibri"/></font>' +
    '<font><sz val="10"/><color rgb="FF586A5E"/><name val="Calibri"/></font>' +
    '<font><b/><sz val="12"/><color rgb="FFFFFFFF"/><name val="Calibri"/></font>' +
    '<font><b/><sz val="11"/><color rgb="FF02661F"/><name val="Calibri"/></font>' +
    '<font><b/><sz val="11"/><color rgb="FFD93125"/><name val="Calibri"/></font>' +
  '</fonts>' +
  '<fills count="4">' +
    '<fill><patternFill patternType="none"/></fill>' +
    '<fill><patternFill patternType="gray125"/></fill>' +
    '<fill><patternFill patternType="solid"><fgColor rgb="FF03832A"/><bgColor indexed="64"/></patternFill></fill>' +
    '<fill><patternFill patternType="solid"><fgColor rgb="FFE6F4EA"/><bgColor indexed="64"/></patternFill></fill>' +
  '</fills>' +
  '<borders count="2">' +
    '<border><left/><right/><top/><bottom/><diagonal/></border>' +
    '<border><left/><right/><top/><bottom style="thin"><color rgb="FFDCE6DB"/></bottom><diagonal/></border>' +
  '</borders>' +
...
function xlsxSheet(R) {
  let n = 0, body = "";
  const merges = [];
  R.forEach(row => {
    n++;
    if (row.k === "gap") { body += '<row r="' + n + '" ht="7" customHeight="1"/>'; return; }
    if (row.k === "pair") {
      body += '<row r="' + n + '">' + xlCell("A" + n, 5, row.a) + xlCell("B" + n, row.danger ? 7 : 6, row.b) + "</row>";
      return;
    }
    const st = row.k === "title" ? 1 : row.k === "meta" ? 2 : row.k === "sec" ? 3 : 4;
    const ht = row.k === "title" ? 28 : row.k === "meta" ? 17 : row.k === "sec" ? 23 : 20;
    body += '<row r="' + n + '" ht="' + ht + '" customHeight="1">' +
      xlCell("A" + n, st, row.a) + xlCell("B" + n, st, "") + "</row>";
    merges.push("A" + n + ":B" + n);
  });

  return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
    '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">' +
    '<dimension ref="A1:B' + Math.max(n, 1) + '"/>' +
    '<sheetViews><sheetView showGridLines="0" tabSelected="1" workbookViewId="0"/></sheetViews>' +
    '<sheetFormatPr defaultRowHeight="15"/>' +
    '<cols><col min="1" max="1" width="38" customWidth="1"/><col min="2" max="2" width="64" customWidth="1"/></cols>' +
...
function buildXlsx() {
  const files = [
    ...
    { name: "xl/workbook.xml", text:
      ...
      '<sheets><sheet name="ТЗ" sheetId="1" r:id="rId1"/></sheets></workbook>' },
    { name: "xl/_rels/workbook.xml.rels", text:
      ...
      "</Relationships>" },
    { name: "xl/styles.xml", text: XL_STYLES },
    { name: "xl/worksheets/sheet1.xml", text: xlsxSheet(excelModel()) }
  ];
  return { name: fileBase() + ".xlsx", bytes: zipStore(files) };
}

/* Запасной формат: в просмотрщике Artifact сохранять .xlsx нельзя,
   разрешён .csv — Excel открывает его так же. */
function buildCsv() {
  const q = s => (/[";\n]/.test(String(s)) ? '"' + String(s).replace(/"/g, '""') + '"' : String(s));
  const lines = excelModel().map(row => {
    if (row.k === "gap") return "";
    if (row.k === "pair") return q(row.a) + ";" + q(row.b || "");
    return q(row.a) + ";";
  });
  /* BOM в начале — иначе Excel читает кириллицу как крокозябры */
  return { name: fileBase() + ".csv", text: String.fromCharCode(0xFEFF) + lines.join(String.fromCharCode(13, 10)) };
}
...
async function downloadXlsx() {
  ...
  if (dl) {
    try {
      await dl.save({ filename: wb.name, data: wb.bytes });
      return { ok: true, name: wb.name };
    } catch (e) {
      const code = e && e.code;
      if (code === "rejected_extension" || code === "extension_not_enabled") {
        const csv = buildCsv();
        try {
          await dl.save({ filename: csv.name, data: csv.text });
          return { ok: true, name: csv.name, csv: true };
        } catch (e2) { return { ok: false, code: (e2 && e2.code) || "unavailable" }; }
      }
      return { ok: false, code: code || "unavailable" };
    }
  ...
  const b = new Blob([wb.bytes], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
  const a = document.createElement("a");
  a.href = URL.createObjectURL(b); a.download = wb.name;
  document.body.appendChild(a); a.click();
  setTimeout(() => { URL.revokeObjectURL(a.href); a.remove(); }, 1500);
  return { ok: true, name: wb.name };
}
...
$("#btnXlsx").addEventListener("click", async () => {
  const b = $("#btnXlsx"); b.disabled = true;
  try {
    const r = await downloadXlsx();
    if (!r.ok) { flash(dlError(r.code), "err"); return; }
    flash(r.csv
      ? "Здесь файл Excel сохранить нельзя, поэтому заявка выгружена таблицей <b>" + esc(r.name) + "</b> — Excel открывает её так же."
      : "Таблица заявки сохранена: <b>" + esc(r.name) + "</b>. Внутри — все графы формы по порядку, без фотографий.", "ok");
  } finally { b.disabled = false; }
});
let mgrWasOpen = null;
window.addEventListener("beforeprint", () => {
  $("#pdate").textContent = "Дата заполнения: " + fillDate();
  const m = $("#mgrBox");
...
/* Перетаскивание файла заявки на страницу — второй вход коммерческого отдела.
   ВРЕМЕННО ОТКЛЮЧЁН вместе с плашкой «Вы из коммерческого отдела?» и кнопкой
   «Скачать заявку»: поставьте true, чтобы вернуть. Кнопка «Загрузить заявку
   клиента» скрыта отдельно, в стилях part-a.html. */
const DROP_TZ = false;

/* Файл, брошенный мимо зон загрузки фото, гасим в любом случае — иначе браузер
   откроет его вместо страницы и заполненная форма пропадёт с экрана. */
...
document.addEventListener("dragover", e => {
  ...
  e.preventDefault();
  if (DROP_TZ) document.body.classList.add("drop-on");
});
